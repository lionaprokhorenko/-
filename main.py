...
async def neuro(update, context):
    ...
    await update.message.reply_text('Нейронные сети - это математические модели, которые имитируют '
...
async def use(update, context):
    await update.message.reply_text('Нейросети в настоящее время используются во многих областях, включая:\n'
...
async def future(update, context):
    await update.message.reply_text('Нейросети имеют огромный потенциал для развития и будут продолжать применяться во многих областях, '
...
async def creation(update, context):
    await update.message.reply_text('Для создания и обучения нейросети нужно выполнить несколько шагов:'
                                    ...
                                    '6. Доработать нейросеть, если необходимо, повторив процесс обучения на новых данных или изменяя ее параметры и архитектуру.\n'
                                    '7. Использовать нейросеть для решения задачи в реальном мире, например, для распознавания цифр на изображениях в приложении для мобильного телефона.'
                                                                            )

async def lib(update, context):
    await update.message.reply_text('Наиболее популярных библиотек для создания нейросетей на Python:'
